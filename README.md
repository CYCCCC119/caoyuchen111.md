...
├── 课程设计报告.md       # 课程设计报告（Markdown 源）
├── 课程设计报告.docx     # 课程设计报告（Word 版）
├── 答辩PPT大纲.md        # 答辩 PPT 逐页大纲
...
>
> **一键启动**：双击项目根目录 `start.bat`，脚本以自身所在目录为基准（相对路径，无硬编码绝对路径），同时拉起后端（:8000）与前端（:5173），首次运行自动 `npm install`，并延时自动打开浏览器。脚本为纯英文、ASCII 编码，可随整个项目目录整体打包迁移。
...
| 五 | 文档撰写与答辩准备 | 设计报告、答辩 PPT 大纲 | ✅ 完成 |

## 设计报告与答辩材料

- `课程设计报告.md`：课程设计报告 Markdown 源文件（精简版，含封面/摘要/目录/7 章正文/参考文献）
- `课程设计报告.docx`：由 Markdown 经 pandoc 转换的 Word 版，可直接提交（封面姓名/学号/班级/指导教师为占位符，需自行填写）
- `答辩PPT大纲.md`：答辩 PPT 逐页大纲（18 页，含标题/要点/讲解备注，照此排版即可）
